...
@Component({
  ...
  styleUrls: ['./detail-view.css']     // <-- იყო styleUrl; უნდა იყოს styleUrls (მრ.)
})
export class DetailView implements OnInit {
  ...
  /** გამოთვლითი getter — აღარ დაგვჭირდება ?. ტემფლეითში */
  get parsedIngredients(): ParsedIngredient[] {
    const list = this.recipe?.ingredients ?? [];
    return list.map((ing) => this.parseIngredient(ing));
  }

  /** მარტივი პარსერი: იღებს [რაოდენობას] [ერთეული] [დასახელება] */
  private parseIngredient(ing: string): ParsedIngredient {
    const raw = (ing ?? '').trim();
    if (!raw) return { raw, name: '' };

    // რიცხვი ან ფრაქცია + სარეკომენდაციო ერთეული + სახელწოდება
    // მაგ: "200 g sugar", "1 1/2 cup flour", "½ tsp salt", "2 ც ცხვრის ხორცი"
